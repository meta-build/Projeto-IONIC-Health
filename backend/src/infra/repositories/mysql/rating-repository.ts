...
import {
  CreateRating,
  LoadRatingByReqId
} from '@/domain/contracts/repos/rating'

import { ObjectType, Repository } from 'typeorm'

export class RatingRepository implements CreateRating, LoadRatingByReqId {
  ...
  async loadByReqId({
    requesterId
  }: LoadRatingByReqId.Input): Promise<LoadRatingByReqId.Output> {
    const ratingRepo = this.getRepository(Rating)
